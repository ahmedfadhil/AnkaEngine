class TenantsController < ApplicationController
      ...
      Tenant.transaction do
        if @tenant.update(tenant_params)
          if @tenant.plan =='premium' && @tenant.payment.blank?
            @payment = Payment.new({email: tenant_params["email"], token: params[:payment]["token"],
                                    tenant: @tenant})
            begin
  ...
  def change
    ...
    Tenant.set_current_tenant_id @tenant.id
    session[:tenant_id] = Tenant.current_tenant_id
    redirect_to home_index_path, notice: "Switch to organization #{@tenant.name}"
  end
